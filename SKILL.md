...
- **Whose look it is.** The house look, unless the repo the document is built in commits its own design system as `.cc/design-tokens.json`: then `scripts/build.sh` applies it and says so ("design system: <file>"). Never edit the skill to change a workspace's look. `apply-design-system/SKILL.md` has the token file, the drop-in steps for a Claude Design export, and the xlsx helper.
...
- `apply-design-system/` -- a workspace's own design system: `ds.py` (find, import a Claude Design export, the LaTeX theme, styled xlsx), `SKILL.md` (the token file and the drop-in steps), `examples/salari-test/` (a placeholder system with a sample PDF and workbook).
